...
export default function NewAudioPlayer(){
    ...
    //Initial state
    if(audioPlayerProps.currentlyPlayingIdx === -1)
        return null;
    ...
    return(
        <div className="md:w-[342px] group transition-[width] duration-500 fixed z-40 overflow-hidden left-6 bottom-6 md:hover:w-[500px]">
            {/* Normal state bg-div */}
            <div 
            ...
            ></div>
            <div className="dark:bg-[rgba(19,19,19,0.2)] dark:backdrop-blur-[4px] dark:backdrop-brightness-105 relative flex items-center justify-between w-full px-6 py-3 group-hover:flex-col rounded-xl">
                {/* Normal state */}
                <div className="flex justify-between w-full">
                    <div className="flex flex-col">
                        {trackName.length > 15?
                            <MarqueeText
                                text={trackName} 
                                textClass={"font-secondary font-semibold text-xl"} 
                                marqueeWidth={"w-[104px] md:w-[150px]"}
                            />
                            :
                            <h6 className={"font-secondary font-semibold text-xl "}>
                                {trackName}
                            </h6>
                        }
                        <div className="flex items-center">
                            <p className="text-base font-secondary">{singer}</p>
                            {isArtistVerified ? (
                        ...
                        <div className="flex-col hidden mt-1 group-hover:flex">
                            <p className="text-sm font-secondary dark:text-light-200">Price</p>
                    ...
                    <div 
                        className="hidden group-hover:block  w-[150px] aspect-square rounded-xl overflow-hidden"
                        style={
                            {
                                backgroundImage: `url(${imgSrc})`, 
                                backgroundPosition: "center", 
                ...
                <div className="items-center justify-center hidden w-full my-5 group-hover:flex">
                    <p className={"mr-4 font-primary text-sm min-w-[39px]"}>
                        {audioPlayerProps.currentProgress}
                    </p>
                    {audioPlayerProps.playerIsLoaded?
                        <div
                            onClick={setProgress} 
                            className="bg-light-300 rounded-md cursor-pointer h-[4px] w-10/12 flex items-center"
                        >
                    ...
                    <p className={"ml-4 font-primary text-sm min-w-[39px]"}>
                        {audioPlayerProps.playerIsLoaded? audioPlayerProps.currentDuration:""}
                    </p>
                    <audio 
                        ref={audioTag} 
                        src={audioSrc} 
                ...
                <div className="flex items-center space-x-10 ">
                    <button onClick={prevSong} className="group-hover:flex hidden items-center justify-center p-2 rounded-full dark:bg-[rgba(19,19,19,0.4)] dark:backdrop-blur-[24px] dark:backdrop-brightness-105">
                    ...
                    <button
						...
						className="h-[50px] w-[50px] bg-primary-300 hover:bg-primary-100 rounded-full flex items-center justify-center"
					>
						<i className={"text-2xl text-light-200 fas " + (audioPlayerProps.isPlaying? "fa-pause":"fa-play")}></i>
					</button>
                    <button onClick={nextSong} className="group-hover:flex hidden items-center justify-center p-2 rounded-full dark:bg-[rgba(19,19,19,0.4)] dark:backdrop-blur-[24px] dark:backdrop-brightness-105">
                        <span className="material-symbols-outlined">
                            skip_next
                        </span>
                    </button>
