...
export default function MemberNft({ memberImage, title, name, role, socialLinks, description, dob, profileSong, setShowModal, currentlyPlaying, setCurrentlyPlaying, idx, setSelectedMember }) {
    ...
    const playSongHandler = () => {
        //Pause the currentlyPlaying song before playing any other audio
    ...
    return (
        <div>
            ...
            <div className="hover:cursor-pointer relative w-[222px] h-[190px] overflow-hidden rounded-t-xl">
                {/* Cover art of NFT */}
                <div className="relative w-full h-full" onClick={()=>{setShowModal(true); setSelectedMember(idx);}}>
                ...
                <div className="z-[1] absolute bottom-0 right-0 p-2">
                    ...
                    <audio ref={audio} className="hidden" src={profileSong} onEnded={resetProgress}></audio>
                </div>
            </div>
            {/* Content provided */}
            <div className="dark:bg-[#1D1D1D] bg-light-100 w-[222px] h-[120px] p-4 rounded-b-xl flex flex-col justify-between">
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center text-xs dark:text-light-300"><span className="mr-1 text-xl material-symbols-outlined">person_pin_circle</span> India</div>
                </div>
            </div>
        </div>
    );
