...
export default function CatalogNav({ currentSelection, setCurrentSelection }) {
	...
	return (
		<div className="flex flex-col items-center justify-center w-full h-[280px] bg-light-300 dark:bg-dark-100">
			<h3 className="text-[#1D1D1D] dark:text-light-100 mt-12 text-5xl font-medium">
				M<span className="font-semibold text-primary-100">x</span> Catalog
			</h3>
			<ul className="flex mt-6 space-x-12 font-semibold">
				<li className={(currentSelection === 1 ? "text-black dark:text-light-100" : "text-[#7B7B7B] hover:text-dark-100 dark:hover:text-light-300") + " cursor-pointer"}>
					<Link href="/mxcatalog/new-releases">New Releases</Link>
				</li>

				<li className={(currentSelection === 2 ? "text-black dark:text-light-100" : "text-[#7B7B7B] hover:text-dark-100 dark:hover:text-light-300") + " cursor-pointer"}>
